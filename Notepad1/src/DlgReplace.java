...
class DlgReplace extends JDialog{
	...
	DlgReplace(Notepad frmNotepad){
		...
		btnReplace.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent ae){
				int p=frmNotepad.ta.getSelectionEnd();
				frmNotepad.ta.setText(new StringBuffer(frmNotepad.ta.getText()).delete(frmNotepad.ta.getSelectionStart(), frmNotepad.ta.getSelectionEnd()).insert(frmNotepad.ta.getSelectionStart(), txtReplaceWith.getText()).toString());
				frmNotepad.ta.setCaretPosition(p);
				find(frmNotepad);
			}
		});
		add(btnReplace);
		...
		btnReplaceAll.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent ae){
				frmNotepad.ta.setText(frmNotepad.ta.getText().replaceAll(txtFindWhat.getText(), txtReplaceWith.getText()));
			}
		});
		add(btnReplaceAll);
	...
	void find(Notepad frmNotepad) {
		String mainString=frmNotepad.ta.getText();
		String subString=txtFindWhat.getText();
		boolean matchCase=chkMatchCase.isSelected();
		String direction;
		int pos=frmNotepad.ta.getCaretPosition();
		if(matchCase==false) {
		...
		if(pos!=-1) {
			frmNotepad.ta.setSelectionStart(pos);
			frmNotepad.ta.setSelectionEnd(pos+subString.length());
			frmNotepad.ta.requestFocus();
		}
		else {
